...
def _(catalog, mo, pd):
    if catalog:
        ...
        _output = mo.ui.table(table_list)
    else:
...
def _(mo):
    return (mo.md("""
    ## Table Metadata — `db.customer_orders`
    """),)
...
def _(catalog, mo):
    if catalog:
        try:
            ...
            _output = mo.md(f"_Error loading table: {e}_")
    return table, _output
...
def _(mo, pd, table):
    if table:
        try:
            ...
            _output = mo.md(f"_Error reading snapshots: {e}_")
    return (_output,)
...
def _(catalog, mo):
    if catalog:
        try:
            ...
            _output = mo.md(f"_Scan failed: {e}_")
    return df, _output
...
def _(catalog, mo):
    if catalog:
        try:
            ...
            _tbl = catalog.load_table("db.customer_orders")
            filtered = _tbl.scan(
            ...
            _output = mo.md(f"_Filtered scan failed: {e}_")
    return (_output,)
...
def _(catalog, mo):
    if catalog:
        try:
            _tbl = catalog.load_table("db.customer_orders")
            arrow_table = _tbl.scan().to_arrow()
            _output = mo.vstack([
                mo.md(
            ...
            _output = mo.md(f"_Arrow conversion failed: {e}_")
    return (_output,)
...
def _(df, mo):
    if df is not None and "error" not in df.columns:
        ...
        _output = mo.ui.table(tier_summary)
    return tier_summary, _output
